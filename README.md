...
TARGETが大きすぎると、下記の「排出確率の仕組み」で使う人口帯に実在する市区町村がほぼ無くなってしまうため、現在は54.5万人程度までのTARGETのみを採用しています。
...
CPU戦を試す場合は、トップ画面で「CPU戦をする」を選びます。表示名、CPU人数、TARGETを選んで「CPU戦を始める」を押すと、すぐにゲーム画面へ移動します。
...
本物のトランプ（A〜10の10種の点数）に寄せた、TARGETだけで決まる固定の確率です。現在の合計人口やHIT回数によって変わることはありません。

1. 「1回のHITの人口の目安」= TARGET ÷ 3 を計算する
2. その範囲を10等分し、どの帯も同じ確率（90% ÷ 10 = 9%ずつ）で引く
3. その目安を超えてTARGET以下の人口の市区町村は、まとめて5%の確率で引く
4. TARGETを超える人口の市区町村は、まとめて5%の確率で引く

現在の合計人口やHIT回数を見て確率を引き直す仕組みではないため、事前画面の「人口カード構成」に表示する内訳は、実際にゲーム中使われる確率とそのまま一致します。新しいTARGETを数値だけ追加しても、この計算式がそのまま働くので、TARGETごとの排出比率を個別に作り直す必要はありません。ただしTARGETが大きすぎると10等分した帯に該当する市区町村が実在しなくなるため、採用するTARGETの人数には上限があります（詳細は上記のTARGET表を参照）。
